...
public class GestorAgenda {
    ...
    public AgendaDiaria aplicarPenalizacion(Penalizacion pen,
        ...
        for (int i = 0; i < tareas.size(); i++) {
            ...
            if (bloquesUsados < horasNuevas) {
                nueva.agregarBloque(new BloqueHorario(
                        tarea.getNombre() + " | Prioridad: " + tarea.getPrioridad(),
                        BloqueHorario.TIPO_TAREA));
                bloquesUsados++;
            } else {
                if (!seccionEspera) {
                    nueva.agregarAviso("-- PUEDE ESPERAR PARA OTRO DIA --");
                    seccionEspera = true;
                }
                nueva.agregarBloque(new BloqueHorario(
                        tarea.getNombre() + " | Prioridad: " + tarea.getPrioridad(),
