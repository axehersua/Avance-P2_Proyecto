...
public class GestorRecomendaciones {
     ...
     * Genera un conjunto de recomendaciones detalladas basadas en:
       ...
       - RF07: Distribucion proporcional del tiempo disponible entre las
         tareas pendientes segun su dificultad relativa al total.
     */
    public void generarRecomendaciones(Estudiante estudiante) {
        ...
        if (pens.size() > 0) {
            ...
            for (int i = 0; i < pens.size(); i++) {
                totalMin += pens.get(i).getMinutosPerdidos();
            }
            System.out.println();
        ...
        // Recomendacion sobre horas disponibles + RF07: distribucion proporcional
        // del tiempo disponible entre las tareas pendientes segun su dificultad.
        if (estudiante.getPerfil().isTieneHorarioFijo()) {
            ...
            ArrayList<Tarea> tareasPendientesTotal = combinarPendientes(criticas, altas, pendientes);
            int tareasPendientes = tareasPendientesTotal.size();
            ...
            // RF07 - Distribuir proporcionalmente el tiempo disponible del dia
            // entre las tareas pendientes, segun el peso de su dificultad.
            distribuirTiempo(tareasPendientesTotal, horasDisp * 60);
            contador++;
        }
    ...
    // Une las listas de tareas criticas, altas y de prioridad media/baja
    // pendientes (sin incluir completadas ni atrasadas) en una sola lista.
    private ArrayList<Tarea> combinarPendientes(ArrayList<Tarea> criticas,
                                                ArrayList<Tarea> altas,
                                                ArrayList<Tarea> pendientes) {
        ArrayList<Tarea> todas = new ArrayList<Tarea>();
        todas.addAll(criticas);
        todas.addAll(altas);
    ...
    /**
     * RF07 - Distribuye el tiempo disponible del dia (en minutos) entre las
     * tareas pendientes de forma proporcional a su dificultad relativa al
     * total de dificultad de todas las tareas pendientes.
     *
     * Formula: minutosTarea = (dificultadTarea / dificultadTotal) * minutosDisponibles
     */
    private void distribuirTiempo(ArrayList<Tarea> tareasPendientes, int minutosDisponibles) {
        if (tareasPendientes.size() == 0 || minutosDisponibles <= 0) {
            return;
        }

        int dificultadTotal = 0;
        for (int i = 0; i < tareasPendientes.size(); i++) {
            dificultadTotal += tareasPendientes.get(i).getDificultad();
        }

        if (dificultadTotal == 0) {
            return;
        }

        System.out.println();
        System.out.println("     Distribucion sugerida de tu tiempo disponible");
        System.out.println("     (proporcional a la dificultad de cada tarea):");

        for (int i = 0; i < tareasPendientes.size(); i++) {
            Tarea t = tareasPendientes.get(i);
            double proporcion = (double) t.getDificultad() / dificultadTotal;
            int minutosAsignados = (int) (proporcion * minutosDisponibles);
            System.out.println("       - " + t.getNombre() + ": " +
                    minutosAsignados + " min (dificultad " + t.getDificultad() + "/5)");
        }
    }
